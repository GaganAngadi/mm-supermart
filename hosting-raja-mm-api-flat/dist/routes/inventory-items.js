...
inventoryItemRouter.post("/", requireAuth, requireRole(writeRoles), async (req, res, next) => {
    try {
        ...
        const total = Number((await prisma.$queryRawUnsafe("SELECT COUNT(*) as count FROM inventory_items"))[0]?.count || 0);
        ...
        await prisma.$executeRawUnsafe("INSERT INTO inventory_items (id,item_code,barcode_number,barcode_type,item_name,item_name_kn,main_category,sub_category,company_name,packing,size,unit,hsn_code,opening_stock,current_stock,purchase_price,cost_price,selling_price,dealer_price,mrp,discount_percent,gst_percent,rack_location,minimum_stock,reorder_quantity,sales_account,purchase_account,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(3))", `inv_${Date.now()}_${Math.random().toString(36).slice(2)}`, itemCode, barcodeNumber, input.barcodeType, input.itemName, input.itemNameKn || null, input.mainCategory || null, input.subCategory || null, input.companyName || null, input.packing || null, input.size || null, input.unit || null, input.hsnCode || null, input.openingStock, input.currentStock, input.purchasePrice, input.costPrice, input.sellingPrice, input.dealerPrice, input.mrp, input.discountPercent, input.gstPercent, input.rackLocation || null, input.minimumStock, input.reorderQuantity, input.salesAccount || null, input.purchaseAccount || null);
