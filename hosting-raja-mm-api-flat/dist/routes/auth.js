...
authRouter.post("/refresh", async (req, res) => {
    ...
    try {
        const payload = jwt.verify(refreshToken, process.env.JWT_REFRESH_SECRET ?? process.env.JWT_SECRET ?? "dev-secret");
...
authRouter.patch("/me/credentials", async (req, res, next) => {
    try {
        ...
        const userToken = jwt.verify(token, process.env.JWT_SECRET ?? "dev-secret");
        ...
        if (input.userId && input.userId !== user.email) {
            ...
            if (existing)
                return res.status(409).json({ message: "This User ID is already used" });
        }
