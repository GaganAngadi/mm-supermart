...
function paymentMethod(value: "Cash" | "Card" | "UPI") {
  if (value === "Cash") return "CASH";
  if (value === "Card") return "CARD";
  return "UPI";
}

function paymentMethodFlexible(value: string) {
  if (value === "Cash" || value === "CASH") return "CASH";
  if (value === "Card" || value === "CARD") return "CARD";
  if (value === "UPI") return "UPI";
  return "CASH";
}

async function getDefaultBranch(shopId: string | undefined) {
...
async function getDefaultCategory(shopId: string | undefined, name = "Grocery") {
  const existing = await prisma.category.findFirst({ where: { shopId, name } });
  if (existing) return existing;
  return prisma.category.create({ data: { shopId, name } });
}

async function syncProduct(input: z.infer<typeof productSyncSchema>, shopId: string | undefined) {
...
async function syncCustomer(input: z.infer<typeof customerSyncSchema>, shopId: string | undefined) {
...
async function syncInventory(input: z.infer<typeof inventorySyncSchema>, shopId: string | undefined) {
  ...
  if (!product) throw new Error(`Product not found for inventory sync: ${input.sku}`);
...
async function syncPayment(input: z.infer<typeof paymentSyncSchema>) {
  const order = await prisma.order.findUnique({ where: { invoiceNo: input.invoiceNo } });
  if (!order) throw new Error(`Sale not found for payment sync: ${input.invoiceNo}`);
...
async function syncSale(input: z.infer<typeof offlineSaleSchema>, shopId: string | undefined) {
...
syncRouter.post("/products", requireSyncAuth, async (req, res, next) => {
  try {
    ...
    const synced = [];
    for (const row of rows) synced.push(await syncProduct(row, req.user?.shopId));
...
syncRouter.post("/customers", requireSyncAuth, async (req, res, next) => {
  try {
    ...
    const synced = [];
    for (const row of rows) synced.push(await syncCustomer(row, req.user?.shopId));
...
syncRouter.post("/inventory", requireSyncAuth, async (req, res, next) => {
  try {
    ...
    const synced = [];
    for (const row of rows) synced.push(await syncInventory(row, req.user?.shopId));
...
syncRouter.post("/payments", requireSyncAuth, async (req, res, next) => {
  try {
    ...
    const synced = [];
    for (const row of rows) synced.push(await syncPayment(row));
...
syncRouter.post("/bulk", requireSyncAuth, async (req, res, next) => {
  try {
    ...
    for (const item of input.items) {
      try {
        if (item.entityType === "sale") await syncSale(offlineSaleSchema.parse(item.payload), req.user?.shopId);
        else if (item.entityType === "product") await syncProduct(productSyncSchema.parse(item.payload), req.user?.shopId);
        else if (item.entityType === "customer") await syncCustomer(customerSyncSchema.parse(item.payload), req.user?.shopId);
        else if (item.entityType === "stockMovement") await syncInventory(inventorySyncSchema.parse(item.payload), req.user?.shopId);
        else if (item.entityType === "payment") await syncPayment(paymentSyncSchema.parse(item.payload));
        else throw new Error(`Unsupported sync entity: ${item.entityType}`);
