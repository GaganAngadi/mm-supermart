...
orderRouter.post("/", requireAuth, async (req, res, next) => {
  try {
    ...
    if (req.user?.shopId) {
      ...
      if (!branch) return res.status(403).json({ message: "Branch does not belong to this shop" });

      const productCount = await prisma.product.count({
        where: { shopId: req.user.shopId, id: { in: input.items.map((item) => item.productId) } }
      });
      if (productCount !== input.items.length) return res.status(403).json({ message: "One or more products do not belong to this shop" });
    }
