...
function generateSku(name: string, barcode?: string) {
...
async function getCategoryId(shopId: string | undefined, categoryName = "Grocery") {
...
async function lookupOpenFacts(host: string, source: string, barcode: string): Promise<OnlineProduct | null> {
...
async function lookupOpenFoodFacts(barcode: string): Promise<OnlineProduct | null> {
  return lookupOpenFacts("world.openfoodfacts.org", "OpenFoodFacts", barcode);
}

async function lookupOpenBeautyFacts(barcode: string): Promise<OnlineProduct | null> {
  return lookupOpenFacts("world.openbeautyfacts.org", "OpenBeautyFacts", barcode);
}

async function lookupOpenProductsFacts(barcode: string): Promise<OnlineProduct | null> {
  return lookupOpenFacts("world.openproductsfacts.org", "OpenProductsFacts", barcode);
}

async function lookupUpcItemDb(barcode: string): Promise<OnlineProduct | null> {
...
async function lookupBarcodeLookup(barcode: string): Promise<OnlineProduct | null> {
...
async function lookupOnlineBarcode(barcode: string) {
...
function barcodeCacheKey(barcode: string) {
  return `barcode-cache:${barcode}`;
}

function isOnlineProduct(value: unknown): value is OnlineProduct {
  const product = value as Partial<OnlineProduct> | null;
  return Boolean(product?.barcode && product?.name);
}

async function getBarcodeCache(barcode: string): Promise<OnlineProduct | null> {
...
async function setBarcodeCache(product: OnlineProduct) {
...
function serializeProduct(product: Awaited<ReturnType<typeof prisma.product.findFirst>> | Record<string, any> | null) {
...
async function saveProduct(input: z.infer<typeof productSchema>, shopId: string | undefined) {
  ...
  if (existing) return prisma.product.update({ where: { id: existing.id }, data, select: productSelect });
  return prisma.product.create({ data, select: productSelect });
}

function barcodeMetadataData(input: { barcode?: string; barcodeType?: string; qrCode?: string; lastPrintedAt?: Date }) {
...
productRouter.get("/barcode/:barcode", requireAuth, async (req, res) => {
  ...
  if (local) return res.json({ source: "local", product: serializeProduct(local), suggestions: [] });

  const cached = await getBarcodeCache(barcode);
  if (cached) return res.json({ source: cached.source ? `${cached.source} cache` : "barcode cache", product: cached, suggestions: [], cached: true });

  const online = await lookupOnlineBarcode(barcode);
  if (online) await setBarcodeCache(online);
...
productRouter.post("/import", requireAuth, requireRole(["Super Admin", "Store Owner", "Manager", "Inventory Staff"]), async (req, res, next) => {
  try {
    ...
    const saved = [];
    for (const input of inputs) saved.push(await saveProduct(input, req.user?.shopId));
